...
first_job_name = 'Accountant'
second_job_name = 'Pilot'
first_location = ''
second_location = ''

# Accessing files in the data_collection folder, please check naming if program does not run
with open("data_collection/" + first_job_name + "-" + first_location + "-skills-bigram.txt", 'r', encoding='utf-8') as list_words:
    first_job_bigram = json.load(list_words)
with open("data_collection/" + second_job_name + "-" + second_location + "-skills-bigram.txt", 'r', encoding='utf-8') as list_words:
